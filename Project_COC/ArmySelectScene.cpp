...
void ArmySelectScene::Initialize() {
    ...
    for (int i = 0; i < totalArmy; i++){
        usedSpace += armyAmount[i];
    }
    
    // TODO 1 (2/8): Add the usedSpace and totalSpace to the label.

    AddNewObject(UISpaceUsage = new Engine::Label("Space: "+to_string(usedSpace)+"/"+to_string(totalSpace)+"", "pirulen.ttf", 30, 1395, 150, 0, 0, 0, 255, 0.5, 0.5));
...
void ArmySelectScene::PlayOnClick(ButtonType type, int id, int spaceCost) {
    if (type == BUTTON_ENTER) {
        ...
        UISpaceUsage->Text = "Space: " +to_string(usedSpace)+ "/" +to_string(totalSpace)+"";
    }
        ...
        if (usedSpace < totalSpace) {
            ...
            armyAmount[id] += 1;
            UISpaceUsage->Text = "Space: " +to_string(usedSpace)+ "/"+to_string(totalSpace)+"";
            UIArmyAmount[id]->Text = to_string(armyAmount[id]);
        }
...
void ArmySelectScene::AddNewArmy(int id, std::string imageName, int spaceCost) {
    ...
    if (id != 3) {
        ...
        AddNewControlObject(btn);
        AddNewObject(new Engine::Label("+", "pirulen.ttf", 40, halfW / 4 + 35 + offsetW, oneThirdH + 50 + offsetH, 0, 0, 0, 255, 0.5, 0.5));
        ...
        AddNewControlObject(btn);
        AddNewObject(new Engine::Label("-", "pirulen.ttf", 40, halfW / 4 + 135 + offsetW + 3, oneThirdH + 50 + offsetH, 0, 0, 0, 255, 0.5, 0.5));
    }
}   
